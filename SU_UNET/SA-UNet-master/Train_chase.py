...
data_location = '/home/amritpal/Downloads/2021/Papers/Cool_Medical_imaging/SA-UNet: Spatial Attention U-Net for Retinal Vessel Segmentation/SA-UNet-master/'
training_images_loc = data_location + 'CHASE/train/image/'
training_label_loc = data_location + 'CHASE/train/label/'
validate_images_loc = data_location + 'CHASE/validate/image/'
validate_label_loc = data_location + 'CHASE/validate/label/'
...
for i in train_files:
    ...
    label = imread(training_label_loc + i.split('_')[0]+"_"+i.split('_')[1].split(".")[0] +"_1stHO.png" ,mode="L")
...
for i in validate_files:
    ...
    label = imread(validate_label_loc +i.split('_')[0]+'_'+ i.split('_')[1].split(".")[0] +"_1stHO.png" ,mode="L")
    old_size = im.shape[:2]  # old_size is in (height, width) format
    delta_w = desired_size - old_size[1]
    delta_h = desired_size - old_size[0]
